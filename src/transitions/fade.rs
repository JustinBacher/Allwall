...
use crate::sources::{
	create_index_buffer, create_pipeline, create_uniform_binds, create_vertex_buffer, Source,
	INDICES,
};
...
impl FadeTransition {
	pub fn new(
		...
		// Create texture bind group layout for 2 textures (from and to)
		...
		// Create uniform bind group layout with 3 bindings (progress, from_aspect, to_aspect)
		...
		// Create 3 separate uniform buffers (one for each binding)
...
impl Transition for FadeTransition {
	...
	fn render(&self, ctx: &Context, current_source: &dyn Source) {
		...
		let from_texture: &Texture = self
			.previous_source
			.as_ref()
			.map(|s| s.texture())
			.unwrap_or(current_texture);

		// Create texture bind group using the stored layout
		...
		queue.write_buffer(
			...
			0,
			bytemuck::cast_slice(&[self.progress]),
		);
		queue.write_buffer(
