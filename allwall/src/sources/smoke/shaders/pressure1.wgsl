// Pressure solver - first iteration (Jacobi method)
// Solves Poisson equation: Laplacian(p) = divergence
// Uses weighted kernel from original ShaderToy implementation

// TODO for MVP improvements:
// - Reduce kernel size for better performance (current 21x21 is expensive)
// - Consider multigrid methods for faster convergence
// - Add relaxation parameter (omega) for SOR method
// - Use shared memory/thread group optimizations
// - Implement convergence check to dynamically stop iterations
...
// Boundary-aware sampling - clamps to edge (Neumann boundary: dp/dn = 0)
fn texel_fetch_div(coord: vec2<i32>) -> vec4<f32> {
    ...
    return textureLoad(divergence_texture, clamped, 0);
}

fn texel_fetch_pre(coord: vec2<i32>) -> vec4<f32> {
    let res = vec2<i32>(uniforms.resolution);
    let clamped = clamp(coord, vec2<i32>(0), res - vec2<i32>(1));
    return textureLoad(pressure_texture, clamped, 0);
}
