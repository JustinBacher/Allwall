		...
		D: Deserializer<'de>,
	{
		...
		impl<'de> Visitor<'de> for MonitorsSpecVisitor {
			...
			fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
				formatter
					.write_str("a string (\"*\", \"any\", or monitor name) or an array of strings")
			}
				...
				E: de::Error,
			{
				...
				if lower == "*" || lower == "any" {
					...
					Ok(MonitorsSpec::Specific(vec![MonitorHandle::new(
						v.to_string(),
					)]))
				}
			}
...
mod tests {
	...
	fn test_deserialize_star() {
		let spec: MonitorsSpec = toml::from_str(r#"monitors = "*""#).unwrap().try_into().unwrap();
		assert_eq!(spec, MonitorsSpec::Any);
	}

	#[test]
	fn test_deserialize_any() {
		let spec: MonitorsSpec = toml::from_str(r#"monitors = "any""#).unwrap().try_into().unwrap();
		assert_eq!(spec, MonitorsSpec::Any);
	}

	#[test]
	fn test_deserialize_single_monitor() {
		let spec: MonitorsSpec =
			toml::from_str(r#"monitors = "DP-1""#).unwrap().try_into().unwrap();
		assert_eq!(
			spec,
	...
	fn try_into<T: for<'de> Deserialize<'de>>(value: toml::Value) -> Result<T, toml::de::Error> {
		T::deserialize(value)
	}
}
